...
In `dynamic` block, I evaluate `delete_after_days` to empty array if it's null and this loop will not run. Otherwise, it will be an array with a single value. After that, I assign the value at line #12 by `lifecycle_rule.value` as `lifecycle_rule` is as same as `dynamic` type name because it's the default iterator name.
...
I prepared two buckets by one with `lifecycle_rule`.
...
Look at `google_storage_bucket.bucket["test2"]` at line #17 that has `lifecycle_rule` at line #30.
